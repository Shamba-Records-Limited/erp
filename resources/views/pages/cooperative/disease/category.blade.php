...
@section('content')
    ...
    @if(has_right_permission(config('enums.system_modules')['Disease Management']['categories'], config('enums.system_permissions')['create']))
        <div class="row">
                ...
                <div class="card">
                    <div class="card-body">
                        <button type="button" class="btn btn-primary btn-fw btn-sm float-right" data-toggle="collapse"
                            ...
                            <span class="mdi mdi-plus"></span>Add Disease Category
                        </button>
                        <div class="collapse @if ($errors->count() > 0) show @endif " id="addDiseaseCategory">
                            <div class="row mt-5">
                                <div class="col-lg-12 grid-margin stretch-card col-12">
                                    <h4>Add Disease Category</h4>
                                </div>
                            </div>


                            <form action="{{ route('cooperative.disease-category.add') }}" method="post">
                                @csrf
                                <div class="form-row">
                                    <div class="form-group col-lg-3 col-md-6 col-12">
                                        <label for="categoryName">Disease Category</label>
                                        <input type="text" name="name"
                                               class="form-control {{ $errors->has('name') ? ' is-invalid' : '' }}"
                                               id="categoryName" placeholder="Dangerous" value="{{ old('name')}}"
                                               required>
                        ...
                        <table class="table table-hover dt">
                            ...
                            @foreach($categories as $key => $category)
                                <tr>
                                    ...
                                    <td>{{$category->name }}</td>
                                    <td>
                                        @if(has_right_permission(config('enums.system_modules')['Disease Management']['categories'], config('enums.system_permissions')['edit']))
                                            <button type="button" class="btn btn-info btn-rounded" data-toggle="modal"
                                                    data-target="#editModal_{{$category->id}}">
