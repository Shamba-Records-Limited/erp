                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="collapse @if ($errors->count() > 0) show @endif " id="addDiseaseCategory">
                            ...
                            <form action="{{ route('cooperative.farm.tracker.cost-break-down.add', $tracker->id) }}"
                                  method="post">
                                @csrf
                                <div class="form-row">
                        ...
                        <table class="table table-hover dt">
                            ...
                            @foreach($costing as $key => $cost)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <form action="{{ route('cooperative.farm.tracker.cost-break-down.delete',$cost->id) }}"
                                              method="post">
                                            @csrf

                                            @if(has_right_permission(config('enums.system_modules')['Farm Management']['farmer_calendar'], config('enums.system_permissions')['edit']))
                                                <button type="button" class="btn btn-info btn-rounded"
                                                        data-toggle="modal"
                                                        data-target="#editModal_{{$cost->id}}">
                                                    <span class="mdi mdi-file-edit"></span>
                                                </button>
                                            @endif

                                            @if(has_right_permission(config('enums.system_modules')['Farm Management']['farmer_calendar'], config('enums.system_permissions')['delete']))
                                        ...
                                        <div class="modal fade" id="editModal_{{$cost->id}}" tabindex="-1"
                                            ...
                                            <div class="modal-dialog modal-dialog-centered" role="document">
                                                <div class="modal-content">
                                                    ...
                                                    <form action="{{route('cooperative.farm.tracker.cost-break-down.edit', [$cost->id])}}"
                                                          method="post">
