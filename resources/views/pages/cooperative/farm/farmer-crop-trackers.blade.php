...
@section('content')
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="collapse @if ($errors->count() > 0) show @endif " id="addFarmerCropStageTracker">
                            ...
                            <form action="{{ route('cooperative.farm.add.tracker.farmer-crop',$farmer_crop_id) }}"
                                  method="post">
    ...
    <div class="row">
        <div class="col-lg-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title">Farmer Crop/Livestock/Poultry Calendars</h4>
                    <div class="table-responsive">
                        <table class="table table-hover dt">
                            ...
                            @foreach($trackers as $key => $tracker)
                                <tr>
                                    <td>{{++$key }}</td>
                                    <td>{{ ucwords( strtolower($tracker->farmer_crop->farmer->user->first_name).' '.strtolower($tracker->farmer_crop->farmer->user->other_names)) }}</td>
                                    ...
                                    <td>
                                        <a href="{{ route('cooperative.farm.tracker.cost-break-down', $tracker->id) }}">{{ $currency.' '.$tracker->cost }}</a>
                                    </td>
                                    <td>

                                        @if(has_right_permission(config('enums.system_modules')['Farm Management']['farmer_calendar'], config('enums.system_permissions')['create']))
                                            <button type="button" class="btn btn-info btn-rounded" data-toggle="modal"
                                                    data-target="#editModal_{{$tracker->id}}">
...
@push('plugin-scripts')
    ...
    <script>
        let url = "{{ route('cooperative.farm.crop-stages-calendar-data', ':farmer_crop_id') }}"
