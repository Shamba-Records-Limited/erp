...
@section('content')
    @if(has_right_permission(config('enums.system_modules')['Procurement']['suppliers'], config('enums.system_permissions')['create']))
        <div class="row">
                ...
                <div class="card">
                    <div class="card-body">
                        <button type="button" class="btn btn-primary btn-fw btn-sm float-right"
                                ...
                                data-target="#addSupplierAccordion"
                                aria-expanded="@if ($errors->count() > 0) true @else false @endif"
                                aria-controls="addSupplierAccordion">
                            <span class="mdi mdi-plus"></span>Add Supplier
                        </button>
                        <div class="collapse @if ($errors->count() > 0) show @endif "
                             id="addSupplierAccordion">
                            <form action="{{ route('supplier.add') }}" method="post">
                                @csrf
                                <div class="form-row">
                                    <div class="form-group col-12">
                                        <h6 class="mb-3">Supplier Type</h6>
                                    </div>
                                    ...
                                    <div class="form-group col-lg-2 col-md-6 col-12 {{ $errors->has('gender') ? '' : 'd-none' }}"
                                         id="genderSelector">
                ...
                <div class="card-body">
                    @if(has_right_permission(config('enums.system_modules')['Customer Management']['crm'], config('enums.system_permissions')['download']))
                        <a class="btn btn-sm btn-info float-right text-white"
                           href="{{ route('cooperative.suppliers.download', 'csv') }}">
                            <i class="mdi mdi-download"></i> CSV
                        </a>

                        <a class="btn btn-sm btn-github float-right text-white"
                           href="{{ route('cooperative.suppliers.download','xlsx') }}"
                           style="margin-right: -5px!important;">
                            <i class="mdi mdi-download"></i> Excel
                        </a>
                        <a class="btn btn-sm btn-success float-right text-white"
                           href="{{ route('cooperative.suppliers.download', 'pdf') }}"
                        ...
                        <table class="table table-hover dt clickable">
                            ...
                            @foreach($suppliers as $key => $customer)
                                ...
                                <tr>
                                    <td>{{++$key }}</td>
                                    <td>
                                        <a href="{{ route('manufacturing.supplier.supplies', $customer->id) }}">{{$customer->title." ".ucwords(strtolower($customer->name))}}</a>
                                    </td>
                                    <td>{{ config('enums')["customer_types"][0][$customer->supplier_type] }}</td>
