...
<table class="table table-items">
    ...
    <tr>
        ...
        <th scope="col" class="border-0">{{ 'Saving ID' }}</th>
        <th scope="col" class="text-left border-0">{{ 'Farmer' }}</th>
        <th scope="col" class="text-left border-0">{{ 'Saving Type' }}</th>
        <th scope="col" class="border-0">{{ 'Amount' }}</th>
        <th scope="col" class="border-0">{{ 'Interest Rate' }}</th>
        <th scope="col" class="border-0">{{ 'Interest' }}</th>
        <th scope="col" class="border-0">{{ 'Amount + Interest' }}</th>
    ...
    @foreach($records as $key => $saving)
        ...
        <tr>
            ...
            <td class="text-left">
                {{ sprintf("%03d", $saving->id) }}
            </td>
            <td class="text-left">
                {{ ucwords(strtolower($saving->first_name .' '.$saving->other_names)) }}
            </td>
            <td class="text-left">
                {{   $saving->saving_type  }}
            </td>
            <td class="text-left">
                {{ $currency.' '.number_format($saving->amount) }}
            </td>
            <td class="text-left">
                {{ $saving->interest_rate.'%' }}
            </td>
            <td class="text-left">
                {{ $currency.' '.number_format($interest) }}
            </td>
            <td class="text-left">
                {{ $currency.' '.number_format($total_amount) }}
            </td>
            <td class="text-left">
                {{ $saving->date_started }}
            </td>
            ...
            <td class="text-left">
                {{ $saving->status == \App\SavingAccount::STATUS_ACTIVE ? 'Active' :  'Withdrawn'}}
            </td>
        </tr>
    @endforeach
    </tbody>
