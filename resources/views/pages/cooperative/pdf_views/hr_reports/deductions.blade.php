            ...
            <div>
                ...
                @else
                    <img src="{{ public_path('assets/images/favicon.png') }}" alt="logo"
                         height="100">
                @endif
                <h4>{{ ucwords(strtolower($user->cooperative->name))}}.<br/> {{ $records['title'] }}</h4>
...
<table id="content" class="table">
    ...
    @foreach($records['deductionData'] as $data)
        <tr>
            ...
            @foreach(config('enums.Months') as $key => $month)
                <td>
                    @if(array_key_exists(--$key,$data["data"] ))
                        @php
                            $total += $data["data"][$key][$data['report_type']];
                            $monthly_total[$key] += $data["data"][$key][$data['report_type']];
                        @endphp
                        {{ number_format($data["data"][$key][$data['report_type']], 2)}}
                    @else
