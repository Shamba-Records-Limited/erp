    ...
    <table class="table table-items">
        <thead>
            <tr>
                ...
                <th scope="col" class="border-0">{{ 'Policy No.' }}</th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="text-left">
                    {{ ucwords(strtolower($trans->subscription->farmer->user->first_name.' '.$trans->subscription->farmer->user->other_names)) }}
                </td>
                <td class="text-left">
                    {{ $trans->subscription->insurance_product->name }}
                </td>
                <td class="text-left">
                    {{ sprintf('%03d', $trans->subscription->id) }}
                </td>
                <td class="text-left">
                    @if($trans->type == \App\InsuranceTransactionHistory::TYPE_REJECT_CLAIM)
                    <badge class="badge badge-danger text-white">Claim Rejected</badge>
                    @elseif($trans->type == \App\InsuranceTransactionHistory::TYPE_INSTALLMENT)
                    <badge class="badge badge-success text-white">Paid Installment</badge>
                    @elseif($trans->type == \App\InsuranceTransactionHistory::TYPE_CLAIM)
                    <badge class="badge badge-info text-white">New Claim</badge>
                    @endif
                </td>
                <td class="text-left">
                    {{ $currency.' '.number_format($trans->amount) }}
                </td>
                <td class="text-left">
                    {{ $trans->comments }}
                </td>
                ...
                <td class="text-left">
                    {{ $trans->createdBy->first_name.' '.$trans->createdBy->other_names }}
                </td>
            </tr>
            @endforeach
        </tbody>
