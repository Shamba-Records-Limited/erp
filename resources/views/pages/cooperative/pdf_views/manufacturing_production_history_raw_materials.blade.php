...
<table class="table table-items">
    ...
    @php
        ...
        $total = 0; $total_quantity = 0; $total_cost = 0;
    @endphp
    @foreach($records as $key => $production)

        @php
            $total += $production->cost;
            $total_quantity += $production->quantity;
            $total_cost += ($production->cost*$production->quantity);
        @endphp
        <tr>
            <td class="text-left">
                {{ ++$key }}
            ...
            <td class="text-left">
                {{  number_format($production->quantity, 2) }}
            </td>
            <td class="text-left">
                {{  number_format($production->cost,2) }}
            </td>
            <td class="text-left">
                {{  number_format($production->cost*$production->quantity,2) }}
            </td>
        </tr>
    @endforeach
    </tbody>
    ...
    <tr>
        ...
        <th colspan="1"> {{ $currency.' '.number_format($total,2) }}</th>
        <th colspan="1"> {{ $currency.' '.number_format($total_cost,2) }}</th>
    </tr>
    </tfoot>
</table>
