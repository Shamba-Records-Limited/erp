...
<table class="table table-items">
    ...
    <tr>
        ...
        <th scope="col" class="text-left border-0">{{ 'Date' }}</th>
        <th scope="col" class="text-left border-0">{{ 'Status' }}</th>
    </tr>
    </thead>
    <tbody>
    @php
    ...
    @foreach($records as $key => $i)
        ...
        <tr>
            ...
            <td class="text-left">
                {{ $currency.' '.number_format($i->amount) }}
            </td>
            <td class="text-left">
                {{ $i->date }}
            </td>
            <td class="text-left">
                {{ $i->status == \App\LoanInstallment::STATUS_PENDING ? 'Pending' :
                    ($i->status == \App\LoanInstallment::STATUS_PAID ? 'Paid' :
                        'Partially Paid') }}
            </td>
        </tr>
    @endforeach
