...
<table class="table table-items">
    ...
    <tr>
        ...
        <th scope="col" class="border-0">{{ 'Quantity Supplied' }}</th>
        <th scope="col" class="border-0">{{ 'Total Value' }}</th>
        <th scope="col" class="border-0">{{ 'Available Quantity' }}</th>
        <th scope="col" class="border-0">{{ 'Available Total Value' }}</th>
    </tr>
    </thead>
    <tbody>
    @foreach($records as $key => $item)
        ...
        <tr>
            ...
            <td class="text-left">
                {{$currency.' '.number_format($item->buying_price) }}
            </td>
            <td class="text-left">
                {{ number_format($item->quantity,2) }} {{$item->unit}}
            </td>
            <td class="text-left">
                {{ $currency.' '.number_format($item->total_value, 2)}}
            </td>
            <td class="text-left">
                {{ number_format($item->available_quantity) }} {{$item->unit}}
            </td>
            <td class="text-left">
                {{ $currency.' '.number_format($item->available_quantity_value, 2)}}
            </td>
        </tr>
    @endforeach
    </tbody>
    ...
    <tr>
        ...
        <th colspan="1">{{$currency.' '.number_format($total_available_quantity_value,2)}}</th>
    </tr>
    </tfoot>
</table>
