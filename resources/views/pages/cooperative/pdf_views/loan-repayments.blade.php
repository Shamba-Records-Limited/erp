...
<table class="table table-items">
    ...
    <tr>
        ...
        <th scope="col" class="border-0">{{ 'Loan Type' }}</th>
        <th scope="col" class="border-0">{{ 'Principle Amount' }}</th>
        <th scope="col" class="border-0">{{ 'Interest Amount' }}</th>
        <th scope="col" class="border-0">{{ 'Interest + Amount' }}</th>
        <th scope="col" class="border-0">{{ 'Installment Amount' }}</th>
        <th scope="col" class="border-0">{{ 'Loan Balance' }}</th>
    ...
    @foreach($records as $key => $lr)
        ...
        <tr>
            ...
            <td class="text-left">
                {{ number_format($lr->principle) }}
            </td>
            <td class="text-left">
                {{ $currency.' '.number_format($interest_amount) }}
            </td>
            <td class="text-left">
                {{ $currency.' '.number_format($interest_amount + $lr->principle) }}
            </td>
            <td class="text-left"> {{ $currency.' '.number_format($lr->installment) }}</td>
            <td class="text-left"> {{ $currency.' '.number_format($lr->balance) }}</td>
            <td class="text-left"> {{ $currency.' '.number_format($lr->balance) }}</td>
            <td class="text-left"> {{ \Carbon\Carbon::parse($lr->installment_date)->format('d-m-Y') }}</td>
            <td class="text-left">
