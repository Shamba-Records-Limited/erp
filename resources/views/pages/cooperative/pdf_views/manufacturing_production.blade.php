...
<table class="table table-items">
    ...
    <tr>
        ...
        <th scope="col" class="border-0">{{ 'Available Quantity' }}</th>
        <th scope="col" class="border-0">{{ 'Unit Selling Price' }}</th>
        <th scope="col" class="border-0">{{ 'Value' }}</th>
        <th scope="col" class="border-0">{{ 'Unit Production Cost' }}</th>
        <th scope="col" class="border-0">{{ 'Margin' }}</th>
    ...
    @foreach($records as $key => $prod)
        ...
        <tr>
            ...
            <td class="text-left">
                {{$prod->available_quantity }} {{$prod->units}}
            </td>
            <td class="text-left">
                {{$currency}} {{ number_format($prod->final_selling_price)}}
            </td>
            <td class="text-left">
                {{$currency}} {{ number_format($prod->final_selling_price * $prod->available_quantity)}}
            </td>
            <td class="text-left">
                {{$currency}} {{ number_format($prod->production_cost,2) }}
            </td>
            <td class="text-left">
                {{$currency}} {{ number_format(($prod->final_selling_price - $prod->production_cost),2) }}
            </td>
            <td class="text-left">
                {{$prod->store }}
            </td>
    ...
    <tr>
        ...
        <th colspan="2"> {{ $currency.' '.number_format($total_margin,2) }}</th>
    </tr>
    </tfoot>
</table>
