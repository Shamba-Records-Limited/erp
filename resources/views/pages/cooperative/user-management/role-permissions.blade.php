...
@section('content')
    @if(has_right_permission(config('enums.system_modules')['User Management']['role_permissions'], config('enums.system_permissions')['create']))
        <div class="row">
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="collapse @if ($errors->count() > 0) show @endif " id="addRoles">
                            ...
                            <form action="{{ route('cooperative.role-permissions.add') }}" method="post">
                                @csrf
                                <div class="form-row">
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover dt">
                            ...
                            <tbody>
                            @php
                                $canEdit = has_right_permission(config('enums.system_modules')['User Management']['role_permissions'], config('enums.system_permissions')['edit']);
                            @endphp
                            @foreach($permissions as $key => $permission)
                                <tr>
                                    <td>{{++$key}}</td>
                                    <td>{{ $permission->internalRole->role }}</td>
                                    <td>

                                        <form action="{{ route('cooperative.role-permissions.edit', $permission->id) }}"
