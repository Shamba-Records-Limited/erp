...
<html lang="en">
<head>
    <title>Farmer Consolidated Report</title>
    ...
    <table class="table mt-5">
        ...
        <tr>
            <td style="text-align:center;padding:0.1rem">
                <p style="font-size:14px;margin:0;">Farmer Consolidated Report For: <strong>{{ $farmer_name }}</strong></p>
            </td>
        </tr>
        <tr>
            <td style="text-align:center;padding:0.1rem">
            ...
            @foreach($records as $record)
                @if($record['label'] == 'Total collections from sales:')
                    ...
                    @foreach($record['value'] as $value)
                        ...
                        <tr class="border-bottom-1">
                            <td class="border-0">
                                <p style="font-size:11px;margin:0;padding:0 0 0 25px;">{{ $value->name }}</p>
                            </td>
                            <td class="border-0">
                                <p style="font-size:11px;margin:0;padding:0 0 0 5px;text-align:right;">{{ number_format($value->amount, 2) }}</p>
                            </td>
                        </tr>
                    @endforeach
                    ...
                    <tr class="border-bottom-1">
                        <td class="border-0">
                            <p style="font-weight:bold;font-size:11px;margin:0;padding:0 0 0 25px;">Total collections:</p>
                        </td>
                        <td class="border-0">
                            <p style="font-size:11px;margin:0;padding:0 0 0 5px;text-align:right;">{{ number_format($collectionsSum, 2) }}</p>
                        </td>
                    </tr>
