...
class InventoryAuctionController extends Controller
{
    ...
    public function list_customers()
    {
        ...
        return view('pages.admin.inventory-auction.customers.index', compact('customers'));
    }
    ...
    public function view_update_customer_details($id)
    {
        ...
        return view('pages.admin.inventory-auction.customers.update-customer-details', compact('customer'));
    }
    ...
    public function view_customer($id, Request $request)
    {
        ...
        return view('pages.admin.inventory-auction.customers.detail', compact('customer', 'tab', 'quotations', 'invoices', 'receipts'));
    }
    ...
    public function list_quotations(Request $request)
    {
        ...
        if ($isAddingQuotation == "1") {
            $customers = Customer::whereNotNull("published_at")->get();

            $finalProducts = FinalProduct::all();

            $milledInventories = MilledInventory::all();

            $draftExists = Quotation::exists();
            if (!$draftExists) {
                $now = Carbon::now();
                $quotationNumber = "QTN";
                ...
                $draftQuotation = Quotation::where("published_at", null)->firstOrFail();
            }
        }
        ...
        return view('pages.admin.inventory-auction.quotation', compact("isAddingQuotation", "viewingQuotationId", "draftQuotation", "customers", "finalProducts", "milledInventories", "quotations"));
    }
    ...
    public function list_invoices(Request $request)
    {
        ...
        if ($isAddingInvoice == "1") {
            ...
            $draftExists = NewInvoice::where("published_at", null)->exists();
            if (!$draftExists) {
                $now = Carbon::now();
                $invoiceNumber = "INV";
                $invoiceNumber .= $now->format('Ymd');
                ...
                $draftInvoice = NewInvoice::where("published_at", null)->firstOrFail();
            }
        }
        ...
        return view('pages.admin.inventory-auction.invoice', compact("isAddingInvoice", "viewingInvoiceId", "draftInvoice", "customers", "finalProducts", "milledInventories", "invoices"));
    }
    ...
    public function list_receipts()
    {
        ...
        $receipts = DB::table('receipts')
            ...
            ->select('receipts.*', 'cust.name as customer_name', 'cust.email as customer_email') // Add desired customer fields
            ->get();
        return view('pages.admin.inventory-auction.receipt', compact("receipts"));
    }
    ...
    public function list_sales()
    {
        ...
        $sales = DB::table('sales')
               ->leftJoin('millers', 'sales.miller_id', '=', 'millers.id')
                ->leftJoin('cooperatives', 'sales.cooperative_id', '=', 'cooperatives.id')
    ->select(
        DB::raw("CASE 
                    WHEN sales.cooperative_id IS NULL THEN 'Miller'
                    WHEN sales.miller_id IS NULL THEN 'Cooperative'
                 END AS entity_type"),
        DB::raw("COALESCE(millers.name, 'N/A') AS miller_name"),
        DB::raw("COALESCE(cooperatives.name, 'N/A') AS cooperative_name"),
    ...
    ->groupBy('entity_type', 'millers.name', 'cooperatives.name')
    ->get();


    $accumulatedSales = DB::table('sales')
    ->select(
        DB::raw("CASE 
                    WHEN sales.cooperative_id IS NULL THEN 'Millers'
                    WHEN sales.miller_id IS NULL THEN 'Cooperatives'
                 END AS entity_type"),
        DB::raw('SUM(sales.paid_amount) as total_paid_amount'),
        DB::raw('SUM(sales.balance) as total_balance'),
        DB::raw('SUM(sales.discount) as total_discount'),
    ...
    ->get();
        ...
        return view('pages.admin.inventory-auction.sales.index', compact('sales','accumulatedSales'));
    }
    ...
    public function view_update_sale($id)
    {
        ...
        return view('pages.admin.inventory-auction.sales.update-sale', compact('sale','categories'));
    }
    ...
    public function view_add_sale()
    {
        ...
        return view('pages.admin.inventory-auction.sales.add-sale', compact('sale','categories'));
    }
