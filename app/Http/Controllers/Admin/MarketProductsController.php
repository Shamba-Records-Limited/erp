...
class MarketProductsController extends Controller
{
    ...
    {
        $products = Product::join('millers', 'products.miller_id', '=', 'millers.id')
                 ->select('products.*', 'millers.name as miller_name')
                 ->get()
               ->toArray();
        ...
        return view('pages.admin.market-auction.products', compact('paginatedProducts','categories'));
    }
